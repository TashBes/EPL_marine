...
#cnd_fl <- paste0("condition_scores_150_", year, ".csv")
...
Transform the CRS to match the ecosystem map. For both the ecosystem map and MPA layer combine the polygons of each ecosystem and protected area together into multipolygons to simplify the data. 
...
eco_mpa_cnd <- cnd %>% 
  ...
  rename(natural = `1`,
         ...
         vsev_deg = `4`) %>% 
  #
  #get the % of the whole within the mpa
  #
  mutate(natural_in_mpa = natural*percent_protected,
         mod_in_mpa =mod_deg*percent_protected,
         sev_in_mpa =sev_deg*percent_protected ) %>% 
  #
  #scale each condition percent by its %degradation score light
  #
  mutate(natural_light = natural,
         mod_deg_light = mod_deg*0.5,
         sev_deg_light = sev_deg*0.3,
         v_sev_deg_light = vsev_deg*0.1) %>% 
  #
  #get the total percentage
  #
  mutate(light_perc = sum(c_across(natural_light:v_sev_deg_light), na.rm = T), 
         .by = ecosystem_type) %>% 
  #
  #get new protection level
  #
  mutate(perc_pro_light = percent_protected*light_perc) %>% 
  #
  #scale each condition percent by its %degradation score medium 
  #
  mutate(natural_med = natural,
         mod_deg_med = mod_deg*0.4,
         sev_deg_med = sev_deg*0.2,
         v_sev_deg_med = vsev_deg*0.05) %>% 
  #
  #get the total percentage
  #
  mutate(med_perc = sum(c_across(natural_med:v_sev_deg_med), na.rm = T), 
         .by = ecosystem_type) %>% 
  #
  #get new protection level
  #
  mutate(perc_pro_med = percent_protected*med_perc)
...
EPL_tbl <- eco_mpa_cnd_2023 %>%
  select(ecosystem_type, percent_protected, perc_pro_light, 
         perc_pro_med, pro_lvl_2019_20, 
         pro_lvl_2019_30, pro_lvl_tash_light,
         pro_lvl_tash_med,
         pro_lvl_lara,
