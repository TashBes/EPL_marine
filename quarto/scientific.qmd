...
The 4 class South African National condition map for 2018 (benthic) and 2011 (pelagic) (prepared by SANBI based on marine cumulative pressure mapping) was used to calculate the degradation scores for each ecosystem inside the MPA's.

This 4 class condition was cross tabulated with the National Marine Ecosystem Map 2023 version, vector feature geodatabase (ESRI file geodatabase) (released ...) curated by SANBI (cite) and the Protected Areas vector feature geodatabase prepared by SANBI. 
...
Subset PA data to only include Marine Protected Areas using TYPE = "Marine Protected Area" and transform the CRS to match the ecosystem map. For both the ecosystem map and MPA layer combine the polygons of each ecosystem and protected area together into multipolygons to simplyfy the data. 
...
eco_mpa_cnd_2023 <- pro_2023 %>% 
  mutate(across(everything(), .fns = ~replace_na(.,0))) %>% 
  mutate(
    protection_level_old_30_2023 = case_when(
  ...
  mutate(
    protection_level_new_30_2023 = case_when(
  ...
  mutate(level_2025 = case_when(
    str_detect(protection_level_new_30_2023, "Well Protected") ~ 4, 
    str_detect(protection_level_new_30_2023, "Moderately Protected") ~ 3, 
    str_detect(protection_level_new_30_2023, "Poorly Protected") ~ 2,
    str_detect(protection_level_new_30_2023, "Not Protected") ~ 1,
    TRUE ~ 0  # otherwise, Least Concern
  )) %>% 
  mutate(level_2023 = case_when(
    str_detect(protection_level_old_30_2023, "Well Protected") ~ 4, 
    str_detect(protection_level_old_30_2023, "Moderately Protected") ~ 3, 
    str_detect(protection_level_old_30_2023, "Poorly Protected") ~ 2,
    str_detect(protection_level_old_30_2023, "Not Protected") ~ 1,
    TRUE ~ 0  # otherwise, Least Concern
  )) %>% 
  
  mutate(difference_2023 = if_else(protection_level_old_30_2023 != protection_level_new_30_2023, 
                              if_else(level_2023 < level_2025, "increase", "decrease"), 
                              "same"))
