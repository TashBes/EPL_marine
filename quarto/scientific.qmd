...
Marine Ecosystem Protection Level assessment with the inclusion of additional steps to utilise condition of ecosystems within protected areas. This assessment is for 2025 and compares results from the new method to those from the 2018 assessment. It reveals the degree to which Protected Area targets have been met for each marine ecosystem type (ecosystem representation). The analysis includes a new method to address the effectiveness of MPAs by down weighting the contribution to targets for areas based on their degradation.
...
cnd_fl <- paste0("condition_mpa_", year, ".csv")
...
cnd_mpa <- st_read(list.files(path="data", pattern=cnd_fl, recursive = T, full.names = T)[1]) 

# add ecosysetm map
##connect to database
con <- sql_con("mem_dwh")

mem <- st_read(dsn = con, query = paste0("SELECT ecosystem_type,realm_primary_getl1, eco_extent_km2_",year, ", geometry FROM dim_poly LEFT JOIN fact_ecosystem ON  fk_poly_b_ecosystem_id = pk_ecosystem_id LEFT JOIN dim_eco_class ON fk_eco_eco_class_id_",year, "  = pk_eco_class_id WHERE version = ", "'", year, "'"))%>%
...
Subset protected area data to only include Marine Protected Areas using TYPE = "Marine Protected Area" and transform the CRS to match the ecosystem map. For both the ecosystem map and MPA layer combine the polygons of each ecosystem and protected area together into multipolygons to simplify the data. 
...
Get the intersection of the MPA's and the ecosystems, and then calculate the proportion of each ecosystem that falls within an MPA.
...
eco_mpa_areas <- eco_mpa %>%
  mutate(Area_under_mpa_km2 = units::drop_units(units::set_units(st_area(.), km^2))) %>%  # calculate area in km²
...
To compare results to the 2018 protection level assessment we compared the original 2018 assessment results **i)** to results of using the 2018 method on the new 2023 map (no changes) **ii)**, to results using the new 2023 map and increasing the targets by 10% each **iii)**, and then to results using the new 2023 map, increasing the targets by 10% each, and using the new condition adjusted method **iV)**.
...
#######################################################################################
cnd <- read.csv(list.files(path="data", pattern=cnd_fl, recursive = T, full.names = T)[1]) %>% 
  select(-X)%>% 
  select(-n) %>% 
  filter(condition != "NA") %>% 
  pivot_wider(names_from = condition, values_from = perc)%>% 
  mutate(total = sum(c_across(`1`:`4`), na.rm = T), .by = ecosystem_type) %>% 
...
eco_mpa_cnd <- cnd %>% 
  ...
  #
  #scale each condition percent by its %degradation score 
  #
  mutate(natural_wd = `1`,
         mod_deg_wd = `2`*0.5,
         sev_deg_wd = `3`*0.3,
  ...
  #
  mutate(total_perc = sum(c_across(natural_wd:v_sev_deg_wd), na.rm = T), 
         .by = ecosystem_type) %>% 
  #
  #get new protection level
  #
  mutate(perc_pro_cnd = percent_protected*total_perc)
...
## calculate protection based on a threshold for 30% (this was not done in the 2019 assessment) 
eco_mpa_cnd_2023 <- pro_2023 %>% 
  mutate(across(everything(), .fns = ~replace_na(.,0))) %>% 
  mutate(
    pro_lvl_2018_20 = case_when(
      percent_protected >= 20 & `1` >= 0.2 ~ "Well Protected", ## if protection is more than 20% classify the protection level category as "Well Protected"
      percent_protected >= 20 & `1` < 0.3 ~ "Moderately Protected",
  ...
  mutate(
    pro_lvl_2018_30 = case_when(
      percent_protected >= 30 & `1` >= 0.3 ~ "Well Protected", ## if protection is more than 20% classify the protection level category as "Well Protected"
      percent_protected >= 30 & `1` < 0.3 ~ "Moderately Protected",
  ...
  mutate(
    pro_lvl_2025_30 = case_when(
      perc_pro_cnd >= 30 & `1` >= 0.3  ~ "Well Protected", ## if protection is more than 20% classify the protection level category as "Well Protected"
      perc_pro_cnd >= 30 & `1` < 0.3 ~ "Moderately Protected",
      perc_pro_cnd >= 20 & perc_pro_cnd < 30 ~ "Moderately Protected", ## ifprotection is more than or equal to 15% and less than 20% classify the protection level category as "Moderately Protected"
      perc_pro_cnd > 0.2 & perc_pro_cnd < 20 ~ "Poorly Protected",## if protection is more than or equal to 0.2% and less than 15% classify the protection level category as "Poorly Protected"
      perc_pro_cnd <= 0.2 ~ "Not Protected" ## if protection is less than or equal to 0.2% classify the protection level category as "Not Protected"
    )) %>% 
  arrange(ecosystem_type) %>% 
  
  ##see how many increased and how many decreased
  mutate(level_2018_20 = case_when(
    str_detect(pro_lvl_2018_20, "Well Protected") ~ 4, 
    str_detect(pro_lvl_2018_20, "Moderately Protected") ~ 3, 
    str_detect(pro_lvl_2018_20, "Poorly Protected") ~ 2,
    str_detect(pro_lvl_2018_20, "Not Protected") ~ 1,
    TRUE ~ 0  # otherwise, Least Concern
  )) %>% 
  mutate(level_2018_30 = case_when(
    str_detect(pro_lvl_2018_30, "Well Protected") ~ 4, 
    str_detect(pro_lvl_2018_30, "Moderately Protected") ~ 3, 
    str_detect(pro_lvl_2018_30, "Poorly Protected") ~ 2,
    str_detect(pro_lvl_2018_30, "Not Protected") ~ 1,
    TRUE ~ 0  # otherwise, Least Concern
  )) %>% 
  mutate(level_2025_30 = case_when(
    str_detect(pro_lvl_2025_30, "Well Protected") ~ 4, 
    str_detect(pro_lvl_2025_30, "Moderately Protected") ~ 3, 
    str_detect(pro_lvl_2025_30, "Poorly Protected") ~ 2,
    str_detect(pro_lvl_2025_30, "Not Protected") ~ 1,
    TRUE ~ 0  # otherwise, Least Concern
  )) %>% 
  
  mutate(difference_2018_30_20 = if_else(level_2018_20 != level_2018_30, 
                              if_else(level_2018_20 < level_2018_30, "increase", "decrease"), 
                              "same"))%>% 
  
  mutate(difference_2018_2025_30 = if_else(level_2018_30 != level_2025_30, 
                              if_else(level_2018_30 < level_2025_30, "increase", "decrease"), 
                              "same"))

EPL_tbl <- eco_mpa_cnd_2023 %>% 
  select(-c(`1`, `2`, `3`, `4`, total,
            level_2018_20, level_2018_30, level_2025_30)) 

write.csv(EPL_tbl, "outputs/EPL_tbl_09_06_2025.csv")
  
EPL_tbl%>% 
  nbaR::nba_tbl_colr(COL = pro_lvl_2025_30) 
...
pro_level <- nbaR::nba_map(pro_level_benth_poly,
                      COLS = NULL, 
                      geometry, 
                      CAP = "Benthic protection level 2025", 
                      FILL = pro_lvl_2025_30)
pro_level+
  geom_sf(data=pro_level_benth_poly, colour = "black", fill = NA)+
  theme(panel.border = element_blank())

ggsave("outputs/pro_2025_map_benth.png", height = 12, width = 18, units = 'cm',  bg = "white", dpi = 350)
...
donut_2025 <- pro_level_benth_poly %>% 
  st_drop_geometry() %>% 
  count(ecosystem_type,pro_lvl_2025_30) %>% 
  pivot_wider(names_from = pro_lvl_2025_30, values_from = n) %>% 
...
pro_level <- nbaR::nba_map(pro_level_benth_poly,
                      COLS = NULL, 
                      geometry, 
                      CAP = "Benthic protection level 2018 using 30%", 
                      FILL = pro_lvl_2018_30)
pro_level+
  geom_sf(data=pro_level_benth_poly, colour = "black", fill = NA)+
  theme(panel.border = element_blank())

ggsave("outputs/pro_2018_30_map_benth.png", height = 12, width = 18, units = 'cm',  bg = "white", dpi = 350)
#st_write(pro_level_benth_poly, "outputs/pro_level.gpkg")
...
donut_2018_30 <- pro_level_benth_poly %>% 
  st_drop_geometry() %>% 
  count(ecosystem_type,pro_lvl_2018_30) %>% 
  pivot_wider(names_from = pro_lvl_2018_30, values_from = n) %>% 
  select(ecosystem_type, `Well Protected`,
         `Moderately Protected`, 
         `Poorly Protected`,
         `Not Protected`)
...
nbaR::nba_plot(DF = donut_2018_30,
         ...
         CHRT = "donut",
         NUM = TRUE,
         LAB = "Benthic protection level 2018 using 30%",
         GRP = FALSE,
         SAVE = "outputs/pro_2018_30_donut_benth")
...
pro_level <- nbaR::nba_map(pro_level_benth_poly,
                      COLS = NULL, 
                      geometry, 
                      CAP = "Benthic protection level 2018 using 20%", 
                      FILL = pro_lvl_2018_20)
pro_level+
  geom_sf(data=pro_level_benth_poly, colour = "black", fill = NA)+
  theme(panel.border = element_blank())

ggsave("outputs/pro_2018_20_map_benth.png", height = 12, width = 18, units = 'cm',  bg = "white", dpi = 350)
#st_write(pro_level_benth_poly, "outputs/pro_level.gpkg")
...
donut_2018_20 <- pro_level_benth_poly %>% 
  st_drop_geometry() %>% 
  count(ecosystem_type,pro_lvl_2018_20) %>% 
  pivot_wider(names_from = pro_lvl_2018_20, values_from = n) %>% 
  select(ecosystem_type, `Well Protected`,
         `Moderately Protected`, 
         `Poorly Protected`,
         `Not Protected`)
...
nbaR::nba_plot(DF = donut_2018_20,
         ...
         CHRT = "donut",
         NUM = TRUE,
         LAB = "Benthic protection level 2018 using 20%",
         GRP = FALSE,
         SAVE = "outputs/pro_2018_20_donut_benth")
